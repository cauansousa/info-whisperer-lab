...
import type { Agent, Library, LLMProvider } from "@/types";
...
export default function Agents() {
  ...
  if (loading) return <div className="p-6"><Skeleton className="h-8 w-48 mb-4" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((i) => <Skeleton key={i} className="h-32" />)}</div></div>;
              ...
              {agent.agent_libraries && agent.agent_libraries.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1">
                  {agent.agent_libraries.map((al) => (
                    <Badge key={al.library_id} variant="outline" className="text-[10px]">{al.libraries?.name || al.library_id.slice(0, 8)}</Badge>
                  ))}
                </div>
              )}
            </div>
